...
router.post("/register", async (req, res) => {
  try {
    ...
    if (!fname || !lname || !email || !password) {
      return res
        .status(422)
        .json({ err: "PLz fill all the field properties " });
...
router.post("/getNutrients", async (req, res) => {
  try {
    const nutrient = await NutritionalFood.find({
...
router.get("/food", async (req, res) => {
  try { 
    const nutrient = await NutritionalFood.find({},{items:1,_id:0});
    const arr=[];
    for(let val of nutrient){
      for(let x of val?.items){// console.log(x._id)
        arr.push({title:x.title,path:x.path});
      }
    }
    // console.log(nutrient)
    res.status(201).send(await arr);
  } catch (err) {
    res.status(404).json({ err: "No user Found" });
  }
});
...
router.post("/saveFeedback", async (req, res) => {
  try {
    const result = new Feedback(req.body);
    await result.save();
    return res.status(201).json({msg:"Thank you for your feedback"});
  } catch (err) {
    console.log(err);
    return res.status(422).json({ err: err+"Ooops something went wrong" });
  }
});
...
router.get("/getFeedback", async (req, res) => {
  try {
    ...
    res.status(404).json({ err: "No feedback Found" });
  }
});
...
router.post("/postMessage", async (req, res) => {
  try {
    const { userId, name, email, question } = req.body;
    if (!name || !email || !userId || !question) {
      return res.status(422).json({ err: "Oops something went wrong" });
    } else {
      const quest = new Question(req.body);
      await quest.save();
      return res.status(201).json({ msg: "Messsage posted Successfully" });
    }
  } catch (err) {
    console.log(err);
    return res.status(422).json({ err: "Error 404" });
  }
});
...
router.post("/getReplies", async (req, res) => {
  try {
    if (!req.body.id) {
      ...
      const user = await Question.find({ _id: req.body.id });
...
router.patch("/addItem", async (req, res) => {
  try {
    const update = await NutritionalFood.updateOne(
      { nutrient: "Vitamin jayraj" },
      { $push: { items: req.body } }
    );
    // console.log(update)
...
router.patch("/addReply", async (req, res) => {
  try {
    ...
    const user = await Question.findById({ _id: id });
    const setReply = await Question.updateOne(
      { _id: id },
      { $push: { replies: { email: email, msg: reply } } }
    );
    // console.log(user)
    return res.status(201).json({ msg: "done" });
  } catch (err) {
